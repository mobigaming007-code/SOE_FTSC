...
function rebuildAttendanceSummaryForPersonDate(personId, dateText, shiftId) {
  ensureSheetWithHeaders('ATTENDANCE', 'AttendanceSummary', SPREADSHEET_STRUCTURE.ATTENDANCE.AttendanceSummary);

  const approvedLogs = findObjects('ATTENDANCE', 'AttendanceLogs', function (row) {
    return String(row.person_id || '') === String(personId || '') &&
      String(row.ngay || '') === String(dateText || '') &&
      String(row.shift_id || '') === String(shiftId || '') &&
      String(row.trang_thai_duyet || '') === 'APPROVED';
  });

  const checkins = approvedLogs
    .filter(function (row) {
      return String(row.loai_cham_cong || '') === 'CHECKIN';
    })
    .sort(function (a, b) {
      const da = parseDateTimeValue(a.thoi_gian);
      const db = parseDateTimeValue(b.thoi_gian);
  ...
  const checkouts = approvedLogs
    .filter(function (row) {
      return String(row.loai_cham_cong || '') === 'CHECKOUT';
    })
    .sort(function (a, b) {
      const da = parseDateTimeValue(a.thoi_gian);
      const db = parseDateTimeValue(b.thoi_gian);
  ...
  const attendanceMarks = approvedLogs
    .filter(function (row) {
      return String(row.loai_cham_cong || '') === 'ATTENDANCE';
    })
    .sort(function (a, b) {
      const da = parseDateTimeValue(a.thoi_gian);
      const db = parseDateTimeValue(b.thoi_gian);
  ...
  const firstAttendanceMark = attendanceMarks.length > 0 ? attendanceMarks[0] : null;
  ...
  if (firstAttendanceMark) {
    soGio = 0;
    soCong = shift ? toNumber(shift.so_cong, 1) : 1;
    status = 'COMPLETED';
    note = 'Đã duyệt chấm công một nút';
  } else if (firstCheckin && lastCheckout) {
    const totalMinutes = minutesBetween(firstCheckin.thoi_gian, lastCheckout.thoi_gian);
    soGio = Math.max(0, Math.round((totalMinutes / 60) * 100) / 100);
    soCong = shift ? toNumber(shift.so_cong, 1) : 1;
    status = 'COMPLETED';
  } else if (firstCheckin && !lastCheckout) {
  ...
  if (shift && firstCheckin) {
    const shiftStart = buildDateTimeFromDateAndTime(dateText, shift.gio_bat_dau);

    if (shiftStart) {
      diTrePhut = Math.max(0, minutesBetween(shiftStart, firstCheckin.thoi_gian));
    }
  }

  if (shift && lastCheckout) {
    const shiftEnd = buildDateTimeFromDateAndTime(dateText, shift.gio_ket_thuc);

    if (shiftEnd) {
      veSomPhut = Math.max(0, minutesBetween(lastCheckout.thoi_gian, shiftEnd));
    }
  }
  ...
  const summaryData = {
    person_id: personId,
    membership_id: firstAttendanceMark
      ? firstAttendanceMark.membership_id
      : (firstCheckin
      ? firstCheckin.membership_id
      : (lastCheckout ? lastCheckout.membership_id : '')),
    ngay: dateText,
    shift_id: shiftId,
    checkin_at: firstAttendanceMark ? firstAttendanceMark.thoi_gian : (firstCheckin ? firstCheckin.thoi_gian : ''),
    checkout_at: lastCheckout ? lastCheckout.thoi_gian : '',
    gio_vao: firstAttendanceMark ? firstAttendanceMark.thoi_gian : (firstCheckin ? firstCheckin.thoi_gian : ''),
    gio_ra: lastCheckout ? lastCheckout.thoi_gian : '',
    gio_vao_tinh_cong: firstAttendanceMark ? firstAttendanceMark.thoi_gian : (firstCheckin ? firstCheckin.thoi_gian : ''),
    gio_ra_tinh_cong: lastCheckout ? lastCheckout.thoi_gian : '',
    so_gio_nghi_ngoi: 0,
    so_gio_tinh_cong: soGio,
    so_cong_quan_tri: soCong,
    checkin_log_id: firstAttendanceMark ? firstAttendanceMark.id : (firstCheckin ? firstCheckin.id : ''),
    checkout_log_id: lastCheckout ? lastCheckout.id : '',
    so_gio: soGio,
    so_cong: soCong,
  ...
  if (existing) {
    updateObjectByRowNumber('ATTENDANCE', 'AttendanceSummary', existing.__rowNumber, summaryData);
    return existing.id;
  }

  summaryData.id = generateId('SUM');
  summaryData.created_at = nowIso();

  appendObjectRowWithLock('ATTENDANCE', 'AttendanceSummary', summaryData);
